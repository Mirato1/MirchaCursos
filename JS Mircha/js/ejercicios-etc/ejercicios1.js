...
const numeroPrimo = (numero = undefined) => {
  ...
  if (numero === 0 || numero === 1)
    return console.error("El número no puede ser 0 ni 1");
...
const convertirGrados = (grados = undefined, unidad = undefined) => {
  ...
  if (typeof unidad !== "string")
    return console.error(`El valor ${unidad} Ingresado, No es un una unidad`);
  ...
  if (unidad === "C") {
    return console.info(
      `${grados}°C = ${(grados * (9 / 5) + 32).toFixed(2)}°F`
    );
  } else if (unidad === "F") {
    return console.info(
      `${grados}°F = ${((grados - 32) * (5 / 9)).toFixed(2)}°C`
    );
  }
};
...
const convertirBinarioDecimal = (numero = undefined, base = undefined) => {
  ...
  if (typeof base !== "number")
    return console.error(`El valor ${base} Ingresado, No es un una unidad`);
